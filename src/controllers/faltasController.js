...
export async function registerFalta(req, res) {
    try {
        const { falta, data, programacao, dia_reuniao, requisicao, obs, materia_prima_id, cortador_id } = req.body; 
        const newFalta = await createFalta( falta, data, programacao, dia_reuniao, requisicao, obs, materia_prima_id, cortador_id);
...
export async function handleUpdateFalta(req, res) {
    try {
        const { id } = req.params;
        const { falta, data, programacao, dia_reuniao, requisicao, obs, materia_prima_id, cortador_id } = req.body;
        const faltaAtualizada = await updateFalta( id, falta, data, programacao, dia_reuniao, requisicao, obs, materia_prima_id, cortador_id );
        if (!faltaAtualizada) {
            return res.status(404).json({ message: "Falta não encontrada" });
        }
