...
export async function getCortadorById(id) {
    const result = await pool.query("SELECT * FROM cortadores WHERE id = $1", [id]);
...
export async function deleteCortador(id) {
    await pool.query("DELETE FROM cortadores WHERE id = $1", [id]);
    return { message: "Cortador deletado com sucesso" };
}